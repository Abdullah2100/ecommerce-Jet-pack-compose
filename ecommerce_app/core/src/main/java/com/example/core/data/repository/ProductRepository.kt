...
class ProductRepository(private val client: HttpClient, private val authDao: AuthDao) :
    ...
    override suspend fun getProduct(pageNumber: Int): NetworkCallHandler =
    ...
    override suspend fun getProduct(storeId: UUID, pageNumber: Int): NetworkCallHandler =
        withContext(Dispatchers.IO) {
            val authData =
                getAuthData()
                    ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
                        ...
                        formData {
                            ...
                            if (productVariants.isNotEmpty())
                                productVariants.forEachIndexed { index, value ->
                                    ...
                                    append(
                                        "productVarients[${index}].percentage",
                                        value.percentage!!
                                    )
                                    append(
                        ...
                        formData {
                            ...
                            if (!productVariants.isNullOrEmpty())
                                productVariants.forEachIndexed { index, value ->
                                    ...
                                    append(
                                        "productVariants[${index}].percentage",
                                        value.percentage!!
                                    )
                                    append(
    ...
    override suspend fun deleteProduct(storeId: UUID, productId: UUID): NetworkCallHandler =
        withContext(Dispatchers.IO) {
            val authData =
                getAuthData()
                    ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
