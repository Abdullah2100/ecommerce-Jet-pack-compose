...
class SubCategoryRepository(private val client: HttpClient, private val authDao: AuthDao) :
    ...
    override suspend fun createSubCategory(data: CreateSubCategoryDto): NetworkCallHandler =
    ...
    override suspend fun updateSubCategory(data: UpdateSubCategoryDto): NetworkCallHandler =
        withContext(Dispatchers.IO) {
            val authData =
                getAuthData()
                    ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
    ...
    override suspend fun deleteSubCategory(subCategoryID: UUID): NetworkCallHandler =
        withContext(Dispatchers.IO) {
            val authData =
                getAuthData()
                    ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
    ...
    override suspend fun getStoreSubCategory(id: UUID, pageNumber: Int): NetworkCallHandler =
        withContext(Dispatchers.IO) {
            val authData =
                getAuthData()
                    ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
