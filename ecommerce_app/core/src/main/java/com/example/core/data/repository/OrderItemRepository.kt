...
import com.example.core.data.dto.UpdateOrderItemStatusDto
...
class OrderItemRepository(private val client: HttpClient, private val authDao: AuthDao) :
    ...
    override suspend fun getMyOrderItemForStoreId(pageNumber: Int): NetworkCallHandler =
    ...
    override suspend fun updateOrderItemStatus(id: UUID, status: Int): NetworkCallHandler =
        withContext(Dispatchers.IO) {
            val authData =
                getAuthData()
                    ?: return@withContext NetworkCallHandler.Error("user is not authenticate ")
