...
class MapRepository(private val client: HttpClient) : IMapRepository {

    override suspend fun getDistanceBetweenTwoPoint(
        origin: Pair<Double, Double>,
        destination: Pair<Double, Double>,
        key: String
    ): NetworkCallHandler = withContext(Dispatchers.IO) {
        val url =
            "https://maps.googleapis.com/maps/api/directions/json?origin=${origin.first},${origin.second}&destination=${destination.first},${destination.second}&key=$key"
